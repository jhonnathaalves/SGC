...
const DocumentosNotas = () => {
  ...
  const recuperarNotas = async () => {
    try {
      const response = await axios.get(api + "/documentos/list/notas", { headers: header });
  ...
  const onFileUpload = (event) => { 
    ...
    try {
      axios.post(api +"/documentos/upload?path=Notas", formData, { headers: headerUpload }) 
        .then((response) => {
          if (response.status === 200) {
            setPage(0);
          ...
          <Table stickyHeader aria-label="sticky table" style={{fontSize:"22px"}}>          
            ...
            <TableBody>             
              {items && items
                ...
                .map((notas) => {
                  return (
                    < TableRow hover role="checkbox" tabIndex={-1} key={notas.id}>
                        < TableCell align="center" component="th" scope="row">{notas.name}</ TableCell>
                        < TableCell align="center" ><DownloadIcon sx={{ color: grey[500] , my: 2 }} onClick={() => handleDowloand(notas.id, notas.name)}/></TableCell>                         
                        {perAdmin?< TableCell align="center" ><DeleteIcon sx={{ color: red[500], my: 2 }} onClick={() => handleRemove(notas.id)}/></TableCell>:""}                  
                    </TableRow>
                  );
                })}
            </TableBody>
...
export default DocumentosNotas