...
const DocumentosImportantes = () => {
  ...
  const recuperarDocumentos = async () => {
    try {
      const response = await axios.get(api + "/documentos/list/docimportantes", { headers: header });
  ...
  const onFileUpload = (event) => { 
    ...
    try {
      axios.post(api +"/documentos/upload?path=DocImportantes", formData, { headers: headerUpload }) 
        .then((response) => {
          if (response.status === 200) {
            setPage(0);
                    ...
                    .map((doc) => {
                      return (
                        < TableRow hover role="checkbox" tabIndex={-1} key={doc.id}>
                            < TableCell align="center" component="th" scope="row">{doc.name}</ TableCell>
                            < TableCell align="center" ><DownloadIcon sx={{ color: grey[500] , my: 2 }} onClick={() => handleDowloand(doc.id, doc.name)}/></TableCell>                         
                            {perAdmin? < TableCell align="center" ><DeleteIcon sx={{ color: red[500], my: 2 }} onClick={() => handleRemove(doc.id)}/></TableCell> :""}                 
                        </TableRow>
                      );
                    })}
                </TableBody>
